...
namespace dotnet_rpg.Services.FightService
{
    public class FightService : IFightService
    {
        ...
        public async Task<ServiceResponse<FightResultDTO>> Fight(FightRequestDTO request)
        {
            ...
            try
            {
                var charcters = await _context.Characters
                    .Include(x => x.Weapon)
                    .Include(x => x.Skills)
                    .Where(x => request.CharacterIds
                    .Contains(x.Id))
                    .ToListAsync();
                ...
                while (!defeated) 
                { 
                    foreach (var attacker in charcters) 
                    { 
                        var opponents = charcters.Where(x => x.Id != attacker.Id).ToList();
